...
import { queueMode } from "@/lib/import-queue";
...
export async function GET() {
  try {
    ...
    const [pending, failedTasks, completedTasks, logs, errors] = await Promise.all([
      ...
      prisma.importTask.findMany({ where: { completedAt: { gte: since } }, select: { processedRows: true, successRows: true, completedAt: true } }),
