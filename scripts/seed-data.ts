...
async function main() {
  ...
  const rows = Array.from({ length: ORDER_COUNT }, (_, index) => {
    ...
    return { 外部编码: `LOAD_${String(index + 1).padStart(6, "0")}`, 收货门店: `压测门店 ${(index % 50) + 1}`, 收件人姓名: `测试收件人 ${(index % 100) + 1}`, 收件人电话: `138${String(10000000 + index).slice(-8)}`, 收件人地址: `测试地址 ${(index % 80) + 1} 号`, SKU物品编码: sku, SKU物品名称: `压测商品 ${((index * 17) % skus.length) + 1}`, SKU发货数量: (index % 5) + 1, SKU规格型号: `${(index % 8) + 1} 规格`, 备注: "V4 load test" };
  });
  ...
  XLSX.writeFile(workbook, path.join(outputDir, "10000-orders.xlsx"));
  ...
  console.log(JSON.stringify({ skuCount: SKU_COUNT, orderCount: ORDER_COUNT, file: "test-data/10000-orders.xlsx", invalidSkuRows: Math.ceil(ORDER_COUNT / 997) }, null, 2));
}
